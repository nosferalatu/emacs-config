...
;; DGF added this function to backup-each-save.el
;; This returns the filename's directory with no colons; the idea
;; is to transform the DOS root drive to be just the drive letter.
;; e.g. c:/emacs/site-lisp/foobar.el -> c/emacs/site-lisp/
;; Without this, backup-each-save-compute-location creates an
;; invalid filename such as c:/emacs/backups/c:/emacs/site-lisp/foobar.el,
;; but with it we get c:/emacs/backups/c/emacs/site-lisp/foobar.el.
(defun file-name-directory-no-colons (filename)
  (let* ((dir (file-name-directory filename))
         (dir-no-root (replace-regexp-in-string ":" "" dir)))
    dir-no-root))

(defun backup-each-save-compute-location (filename)
  (let* ((containing-dir (file-name-directory-no-colons filename))
